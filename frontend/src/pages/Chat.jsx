...
export default function Chat() {
    ...
    const connectToWs=()=>{
        const ws=new WebSocket('wss://messagewebapp.onrender.com');
        
        setWs(ws)
        ws.addEventListener('message', handleMessage);
        ws.addEventListener("close", () => {
            setTimeout(()=>{
                console.log("Disconnected....trying to establish connection")
                connectToWs();
            }, 2000); 
        })
    }

    function peopleOnline(peoples){
        const people={}
        peoples.forEach(p=>people[p.userId]=p.username)
        setOnlinePeople(people)
    }

    function handleMessage(ev) {
      const mssgData = JSON.parse(ev.data);
      console.log({ username, ev, mssgData });
    
      if ('online' in mssgData) {
        peopleOnline(mssgData.online);
      } else if ('text' in mssgData) {
        if (mssgData.sender ===selectedUserId) {
          console.log({selectedId :selectedUserId})
          setMessages(prev => [...prev, { ...mssgData }]);
        }
      }
    }
